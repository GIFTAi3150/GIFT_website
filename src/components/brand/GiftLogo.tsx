// Inlined GIFT logo so the two fills can be themed per page.
// Keeps the same visual silhouette as /public/GIFT_logo.svg — same paths,
// same viewBox — but the hardcoded `.cls-1` / `.cls-2` fills are now props.

import type { CSSProperties } from 'react';

interface GiftLogoProps {
  /** Outer "shield/petal" fill. Default: original brand dark-green. */
  shieldFill?: string;
  /** Inner "G" fill. Default: white. */
  innerFill?: string;
  className?: string;
  style?: CSSProperties;
  title?: string;
...
export default function GiftLogo({
  shieldFill = '#234a2d',
  innerFill = '#fff',
  className,
  style,
