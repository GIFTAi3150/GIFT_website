...
export const LOGO_WALL: {
  // Phrases are joined with <wbr> so the narrow column only breaks between them.
  trust: { lead: string[] };
  note?: string;
  tiles: LogoTile[];
} = {
  // 2026-09-25: 「200社以上」は誇張との指摘(上長)で数字表記を廃止。実数が確定するまで数値は出さない。
  trust: {
    lead: ['私たちを', '信頼して', 'くださる', '企業様'],
  },
