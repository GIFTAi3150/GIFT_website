...
void main() {
  ...
  vec3 background = mix(vec3(0.0431373,0.0627451,0.1254902),vec3(1.0),uLight);
  ...
  // Keep the GIFT cutout anchored to the hero inside the viewport-fixed waves.
  vec2 maskUv = vec2(cellUv.x,(center.y-uMaskPlacement.x)/uMaskPlacement.y);
  if (uLight < 0.5 && maskUv.y >= 0.0 && maskUv.y <= 1.0 && texture2D(uMask,maskUv).r > 0.5) {
    gl_FragColor = vec4(background,0.0); return;
  }
  ...
  if (uLight > 0.5) {
    // Low-contrast ink on white; the same ripple field deepens its blue on interaction.
    float response = smoothstep(0.02,0.6,charge);
    vec3 ink = mix(vec3(0.82,0.87,0.94),vec3(0.45,0.59,0.8),response);
    float strength = 0.18+0.14*level+0.25*response;
    gl_FragColor = vec4(mix(background,ink,coverage*strength),1.0);
    return;
  }
...
export function createFlow(canvas, options = {}) {
  ...
  const hero=light?null:document.querySelector('.hero'), surface=options.surface??document.body;
  ...
  let width=1,height=1,frame=0,last=0,time=0,intro=(options.paused||light)?1.66:0;
  let paused=Boolean(options.paused),lost=false,destroyed=false;
  let visible=!light||!('IntersectionObserver' in window);
  let heroHeight=1,heroTop=0,scrollFrame=0;
  ...
  function initialize() {
    scene=program(fragmentSource,['uResolution','uMaskPlacement','uGrid','uTime','uIntro','uLight','uMask','uCharges']);
    if(!light){blur=program(blurSource,['uSource','uStep','uExtract']);composite=program(compositeSource,['uScene','uGlow']);}
  ...
  function render() {
    ...
    pass(scene,light?null:targets[0]);bindTexture(maskTexture,0);bindTexture(chargeTexture,1);
    const u=scene.uniforms;
    gl.uniform1i(u.uMask,0);gl.uniform1i(u.uCharges,1);gl.uniform2f(u.uResolution,width,height);
    gl.uniform2f(u.uMaskPlacement,heroTop-window.scrollY,heroHeight);gl.uniform1f(u.uLight,light?1:0);
  ...
  function drawMask() {
    ...
    if(light||hero?.querySelector('.hero-wordmark')){
      // The HTML wordmark stays crisp above the waves; no cutout mask is needed.
      bindTexture(maskTexture,0);
      gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,1,1,0,gl.RGBA,gl.UNSIGNED_BYTE,new Uint8Array([0,0,0,255]));
      return;
    }
    const heroRect=hero?.getBoundingClientRect();
    heroHeight=Math.max(1,heroRect?.height??height);
    heroTop=(heroRect?.top??0)+window.scrollY;
  ...
  function resize() {
    ...
    const mobile=width<=800,pixelBudget=light?(mobile?500000:1100000):(mobile?850000:1800000);
    const scale=Math.min(window.devicePixelRatio||1,light?1.25:1.5,Math.sqrt(pixelBudget/(width*height)));
  ...
  function tick(now) {
    frame=0;if(paused||!visible||document.hidden||lost||destroyed) return;
    const elapsed=now-last;
    if(elapsed>=(light||width<=800?30:20)) {
  ...
  function start() {if(!frame&&!paused&&visible&&!document.hidden&&!lost&&!destroyed){last=performance.now();frame=requestAnimationFrame(tick);}}
  ...
  function scroll() {
    // Paused waves still need to follow the hero when the document moves.
    if(light||!paused||scrollFrame||document.hidden||lost||destroyed) return;
  ...
  function interact(event,click=false) {
    if(paused||!visible||document.hidden||lost||destroyed||event.target.closest?.('a,button,input,nav,dialog')) {leave();return;}
  ...
  function visibility() {leave();if(document.hidden)stop();else start();}
  ...
  const visibilityObserver=light&&'IntersectionObserver' in window?new IntersectionObserver(([entry])=>{
    visible=entry.isIntersecting;
    if(visible)start();else stop();
  }):null;
  visibilityObserver?.observe(surface);
  if(!light)document.fonts?.ready.then(()=>{if(!destroyed&&!lost){drawMask();render();}});
  start();
  return {
    setPaused(value){paused=Boolean(value);if(paused){stop();intro=1.66;render();}else start();},
    destroy(){
      stop();destroyed=true;available(false);observer?.disconnect();visibilityObserver?.disconnect();
      ...
      if(!lost){for(const target of targets){gl.deleteFramebuffer(target.framebuffer);gl.deleteTexture(target.texture);}gl.deleteTexture(maskTexture);gl.deleteTexture(chargeTexture);gl.deleteBuffer(buffer);for(const shader of [scene,blur,composite])if(shader)gl.deleteProgram(shader.handle);}
    }
  };
}
