...
### ① `dev` ブランチが本番です（`main` ではありません）

Vercel の Production Branch が `dev` に設定されています。
**`dev` にマージした瞬間に www.gift-inc.org が更新されます。**
`main` は使われておらず、中身も古いままです。

作業は `dev` からブランチを切って、PR で `dev` に戻す流れです。
...
`dev` が本番という変則運用のままです。いつか整理が必要ですが急ぎではありません。
