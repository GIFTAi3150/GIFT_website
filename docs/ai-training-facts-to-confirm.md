...
| 2 | 助成金名 | 人材開発支援助成金（仮置き） | `PRICING.subsidized.body` | 実際に活用を想定している助成金メニュー名を確定 |
...
| 5 | 対象企業の条件 | 未記載（FAQで助成金の審査がある旨のみ言及） | `FAQ.items[2].answer`（助成金Q&A） | 中小企業要件・雇用保険適用事業所要件など対象条件を確認し、必要ならFAQに追加 |
| 6 | 最少催行人数 | 目安3名〜（1名からも相談可、という表現） | `FAQ.items[1].answer` | 実際の最少催行人数を確認 |
| 7 | 期間・回数 | 全4回・1〜2ヶ月程度の目安 | `STEPS.items`（研修実施の説明）/ `FAQ.items[3].answer` / `PRICING.regular.note` | 標準の回数・期間を確定 |
